...
This package was developed based on the CMS/HHS Published DIY SAS model for the HHS-HCC risk adjustment programs, published at https://www.cms.gov/cciio/resources/regulations-and-guidance#Health%20Market%20Reforms. The current version was based on the DIY model published January 23, 2026, relying on the “instructions” [^1] and “technical details” [^2]. We have incorportated EDGE guidance on risk adjustment eligible CPT/HCPCs changes for 2026. We have made our best efforts to replicate the logic found in the CMS-published SAS algorithm in T-SQL assuming a Microsoft SQL Server environment. For benefit years where HHS has not yet issued a DIY model , the most recently published coefficients found in rulemaking were used. 
...
If you find any errors or suspected errors, please report them to rebecca@evensun.com or wesley@evensun.co. If you would like assistance with interpreting the results of this model, you may reach out to the same addresses. Please do not share any protected health information when reaching out.
...
If you have existing tables in an earlier input table format you may use the sql view code to translate the prior column names to current column names.
...
##### Pharmacy Claims
- The MemberID, claimnumber, NDC, filleddate, paiddate, billedamount, allowedamount, and paidamount columns are required. All other fields are optional and are not used by the model.

Once these four tables have been populated, you can then run the “DIY Model Script.sql” script. Update the use statement on line 4 with the name of the database you are using. Set the benefit year to the model year you wish to use for calculating risk scores. This model package has benefit years 2022 to 2027 available. Ordinarily, you will want to use the benefit year corresponding to the incurred date and eligibility date of the claims data you have populated, as this will then correspond to your EDGE server submissions. However, you may wish to use other model years for analyzing impacts of risk adjustment model and coefficient changes on your risk scores.
...
[^1]: [https://www.cms.gov/files/document/cy2025-diy-instructions-01-23-2026.pdf]
[^2]: https://www.cms.gov/files/document/cy2025-diy-tables-01-23-2026.xlsx
